...
At it's core we are looking to take an existing array of employees convert each of the Employees into a new data structure. If we start there we need a function that will process the provided Employees list and make a new list with alternate Employee data.
...
function processAllEmployees(employeeList) {
    ...
    for (let i = 0; i < employeeList.length; i++) {
        ...
        const newEmployee = convertEmployeeData(employeeInfo);
...
function convertEmployeeData(employeeBase) {
    const employeeConverted = {};
    return employeeConverted;
}
```

Call to the new function `processAllEmployees` and see if it works.
